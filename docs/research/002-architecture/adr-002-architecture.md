...
No BFF-level authorization logic. The BFF proxies requests and validates tokens, but all permission decisions are made by Kubernetes RBAC.
...
The BFF's only provisioning action is committing `HelmRelease` resources to the main gitops repo. Flux reconciles these into Cozystack tenants.

```
User clicks "Create Tenant" in portal
  --> BFF validates token + RBAC
  --> BFF commits HelmRelease YAML to gitops repo
...
Tenant removal follows the same path in reverse: BFF removes the HelmRelease from git, Flux prunes the resources.
...
- Simple BFF -- the backend is a thin layer: validate token, commit YAML, proxy K8s API. No user database, no session store, no permission tables

### Negative

- Scaling ceiling at ~250 tenants before JWT/proxy tuning is needed (see [Scaling Limits](./scaling-limits.md))
- No offline tenant provisioning -- requires git repo and Flux to be available
...
| Flux reconciliation lag frustrates users | Flux reconciles in seconds for < 1,000 HelmReleases. Add webhook receivers for faster feedback if needed. |
...
- **Issue #6 (Onboarding)**: Onboarding wizard submits to BFF, which commits tenant HelmRelease. Real-time status via Kubernetes API watch through Refine's `liveProvider`.
